		...
		Pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
			   ...
			   // indirect specular ONLY
				   ...
				   fixed3 envReflection = /*texCUBE(_Enviroment, i.worldRefl).rg*/samplePanoramicLOD(_Enviroment, i.worldRefl, 0);
			   ...
			   //indirect IBL including specular and diffuse
				...
				float3 reflDir = reflect(-worldViewDir, worldNormal);
					...
					float3 PrefilteringSum = samplePanoramicLOD(_Enviroment, uvlod.xyz, uvlod.w); /*texCUBElod(_Enviroment, uvlod)*/;
					...
					indirectSpec = PrefilteringSum * EnvBrdf;
					...
					float3 indirectDiffLD = samplePanoramicLOD(_Enviroment, uvlod.xyz, uvlod.w); /* texCUBElod(_Enviroment, uvlod)*/

					half3 indirectDiffDFG = sampleLUT.b;
					half3 indirectDiff = albedo * indirectDiffDFG * indirectDiffLD;
