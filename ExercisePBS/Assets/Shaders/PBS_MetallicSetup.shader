		...
		SubShader
	{
		...
		Pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				float3 albedo = _Albedo - 1e-2;// tex2D(_Diffuse, i.uv.xy).rgb;
				float3 diffCol = max(1e-2, albedo * (1 - _Metallic));

				half3 specular = 0;
				half3 specularF0 = lerp(0.04, albedo, _Metallic);//metallic set up without specTex setup
