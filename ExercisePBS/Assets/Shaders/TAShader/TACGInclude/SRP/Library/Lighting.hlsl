...
half3 VertexLighting(float3 positionWS, half3 normalWS)
{
    ...
    int vertexLightStart = _AdditionalLightsCount.x;
    int vertexLightEnd = min(_AdditionalLightsCount.y, unity_LightIndicesOffsetAndCount.y);
    for (int lightIter = vertexLightStart; lightIter < vertexLightEnd; ++lightIter)
    {
        Light light = GetAdditionalLight(lightIter, positionWS);
